...
/**
 * Returns true when an error indicates a transient failure safe to retry.
...
/**
 * Runs an async function with exponential-backoff retry on transient errors.
 * @param {Function} fn - The async function to execute.
 * @param {number} maxAttempts - Maximum number of attempts (default 4).
 * @param {number} baseDelayMs - Initial delay in milliseconds (default 5000).
 * @return {Promise} Resolves with the function result.
 */
...
/**
 * Parses a JSON string returned by Gemini, stripping any markdown code fences.
...
/**
 * Calls Gemini Vision to produce a structured character-card JSON for the avatar.
 * @param {GoogleGenAI} ai - The Gemini AI client.
 * @param {string} model - The model name to use.
 * @param {string} avatarB64 - Base-64 encoded avatar image data.
 * @param {string} mimeType - MIME type of the avatar image.
 * @return {Promise} Resolves with the character card and token usage.
 */
...
/**
 * Asks Gemini to identify recurring objects, animals, and locations across all story pages.
 * @param {GoogleGenAI} ai - The Gemini AI client.
 * @param {string} model - The model name to use.
 * @param {string} childName - The main character's name.
 * @param {string} synopsis - The story synopsis.
 * @param {Array} draftPages - The draft story pages.
 * @return {Promise} Resolves with the extracted entities and token usage.
 */
...
/**
 * Generates per-page scene illustration prompts using Gemini.
 * @param {GoogleGenAI} ai - The Gemini AI client.
 * @param {string} model - The model name to use.
 * @param {string} storyTitle - The story title.
 * @param {string} titleEnglish - The English transliteration of the title.
 * @param {string} synopsis - The story synopsis.
 * @param {string} childName - The main character's name.
 * @param {number} childAge - The child's age.
 * @param {string} childGender - The child's gender.
 * @param {Array} draftPages - The draft story pages.
 * @param {CharacterCard} card - The character card for the protagonist.
 * @param {number} numPages - Total number of pages.
 * @param {Array} commonContextEntities - Shared story entities for consistency.
 * @return {Promise} Resolves with the raw scene items and token usage.
 */
 ...
 * Reads story data, generates character cards and scene prompts, then writes
 * scene documents back to Firestore.
 * @param {string} storyId - The Firestore document ID of the story.
 * @return {Promise<void>} Resolves when all scenes have been written.
 */
export async function runScenePipeline(storyId: string): Promise<void> {
